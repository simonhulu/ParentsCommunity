...
//    emojis_ = [[NSDictionary dictionaryWithContentsOfFile:plistPath] copy];
    NSString *plistPath = [[NSBundle mainBundle] pathForResource:@"iemojis" ofType:@"plist"];
    emojis_  = [[NSArray arrayWithContentsOfFile:plistPath] copy];
  }
  return emojis_;
}
...
//    [self addSubview:self.segmentsBar];
    ...
    NSUInteger numberOfPages = [self numberOfPagesForframeSize:CGSizeMake(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - pageControlSize.height)];
...
- (NSUInteger)numberOfPagesForframeSize:(CGSize)frameSize {
