...
-(id) initWithTabItems:(NSArray*)tabBarItemsArray height:(float)tbHeight backgroundImage:(UIImage*)bgImage{
    if ((self = [super init])) {
        ...
        self.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height-20-tabBarHeight-44, 320, tabBarHeight);
...
-(void)showViewControllerAtIndex:(NSInteger)index
{
    ...
    if(button)
    {
        UIViewController *viewController = [[self.tabBarItemArray objectAtIndex:index] viewController] ;
